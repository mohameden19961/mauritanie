...
export default function Cuisine() {
  ...
  return (
    <>
      ...
      <PageHeader
        title={t('Cuisine')}
        description={t("La cuisine mauritanienne est un métissage de saveurs africaines, arabes et berbères. Entre plats en sauce, pâtisseries au miel et le fameux thé à la menthe, elle raconte l'histoire d'un peuple fier et hospitalier.")}
      />

      <section className="section">
        <div className="container">
          <div className="section-title">
            <h2>{t('Plats et traditions culinaires')}</h2>
            <p>{t('Découvrez les mets qui font la richesse gastronomique de la Mauritanie.')}</p>
          </div>
          <div className="cards-grid">
            {MAURITANIA.cuisine.map((item, i) => (
              <div key={i} className="info-card">
                <figure style={{ margin: '0 0 18px' }}>
                  <img className="card-image" src={item.image} alt={t(item.name)} />
                </figure>
                <h3>{t(item.name)}</h3>
                <span className={`badge ${badgeClass(item.type)}`}>{t(item.type)}</span>
                <p style={{ marginTop: 12 }}>{t(item.desc)}</p>
              </div>
            ))}
          </div>
        </div>
