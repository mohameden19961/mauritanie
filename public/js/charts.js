(function() {
  ...
  Object.keys(chartMap).forEach(function(id) {
    ...
    if (!el) return;
    var cfg = chartMap[id];
    var container = el.closest('.chart-container');
    var front = container ? container.querySelector('.flip-front') : el;
    if (!front) return;
    ...
    if (cfg.type === 'doughnut' || cfg.type === 'pie') {
      ...
      var c = cfg.colors ? cfg.colors[0] : (color(cfg.colorKey) || '#0D8A3C');
      var ds = {
        label: cfg.label || '',
        data: cfg.data.map(function(d) { return d[cfg.valueKey]; }),
      ...
      if (cfg.type === 'line') {
        ds.fill = true; ds.tension = 0.4; ds.pointRadius = 4;
        ds.pointBackgroundColor = c; ds.backgroundColor = c + '22';
      }
      if (cfg.type === 'bar') { ds.borderRadius = 4; }
      datasets = [ds];
    }
    ...
    var chart = new Chart(ctx, {
      ...
      data: { labels: cfg.data.map(function(d) { return d[cfg.labelsKey]; }), datasets: datasets },
      options: opts
    });
    createdCharts[id] = chart;

    if (container) {
      var backP = container.querySelector('.flip-back p');
